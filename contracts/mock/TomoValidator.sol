...
contract TomoValidator {
    ...
    constructor (
        ...
        for (uint256 i = 0; i < _candidates.length; i++) {
            ...
            ValidatorState storage validatorState = validatorsState[_candidates[i]];
            validatorState.owner = _firstOwner;
            validatorState.isCandidate = true;
            validatorState.cap = _caps[i];
    ...
    function propose(address _candidate) external payable onlyValidCandidateCap onlyNotCandidate(_candidate) {
        ...
        ValidatorState storage validatorState = validatorsState[_candidate];
        validatorState.owner = msg.sender;
        validatorState.isCandidate = true;
        validatorState.cap = cap;
