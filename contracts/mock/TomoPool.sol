...
contract TomoPool {
    ...
    uint256 public maxCap = 5100000 ether;

    uint256 public stakerWithdrawDelay = 10; // = 96 * BLOCK_PER_EPOCH = 96 epochs = 2 days
    uint256 public candidateWithdrawDelay = 100;// 1440 * BLOCK_PER_EPOCH;//1440 epochs = 30 days
    ...
    bool public isRequireReferralLock = false;
    ...
    function withdrawStakeByTeam(address _staker, uint256 _blockNumber) public onlyTeam {
        require(_blockNumber > 0);
        if (candidateStatus >= PROPOSED_STATUS) {
            require(block.number >= _blockNumber);
            ...
            delete indexes[_blockNumber];
            teamAddr.transfer(cap);
            emit Withdraw(teamAddr, _blockNumber, cap);
        }
    }
    ...
    function withdrawAllRewardsOfStakerByTeam(address payable _staker) public onlyTeam {
        require(EpochsAtWhichCapChange[_staker].length > 0);
        fillRewardsPerEpoch();
        uint256 lastWithdrawEpoch = getLastWithdrawEpochOfStaker(_staker);
        uint256 _ce = currentEpoch();
        ...
        if (_rewardAmount > 0) {
            teamAddr.transfer(_rewardAmount);
            emit WithdrawReward(teamAddr, _rewardAmount);
            TotalRewardWithdrawn = TotalRewardWithdrawn.add(_rewardAmount);
        }
        StakerRewardWithdrawState[_staker] = _ce.sub(2);
    }
    ...
    function withdrawRewardStakerOneHundredEpochByTeam(address payable _staker) public onlyTeam {
        require(EpochsAtWhichCapChange[_staker].length > 0);
        fillRewardsPerEpoch();
        uint256 lastWithdrawEpoch = getLastWithdrawEpochOfStaker(_staker);
        uint256 _ce = currentEpoch();
